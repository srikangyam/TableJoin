 ...
 */
public class MapperDept extends Mapper<LongWritable, Text, IntWritable, Text> 
{
    protected void map(LongWritable key, Text empD, Context context)
              throws IOException, InterruptedException
    {
        String[] str = empD.toString().split(",");
        int mapK = Integer.parseInt(str[0]);
        context.write(new IntWritable(mapK), new Text(str[1]));
    }
}